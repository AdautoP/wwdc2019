...
@objc(Book_Sources_ThirdPageController)
public class ThirdPageController: UIViewController, PlaygroundLiveViewMessageHandler, PlaygroundLiveViewSafeAreaContainer {
    /*
    ...
    override public func viewDidLoad() {
        ...
        if let view = self.view as! SKView? {
            ...
            let scene = LastScene(size: CGSize(width: view.frame.size.width, height: view.frame.size.height))
...
//            view.showsPhysics = true
    ...
    func startInsemination(){
        if let view = view as? SKView{
            if let scene = view.scene as? LastScene{
                scene.startInsemination()
            }
        }
    }
}
